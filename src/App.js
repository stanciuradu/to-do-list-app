      ...
      <div className='render'>
          ...
          {/* daca variabila este true, atnci afiseaza pe ecran componenta AddToDoForm ,altfel afiseza nimic*/}
          {
            this.state.isFormDisplay
              ?<AddToDoForm handleUpdateToDos={(newTodos)=>this.handleUpdateToDos(newTodos)}/>
              :null
          }

          {/* randerz componenta todolist, careia ii trimit ca proprietatil todo-urile din state */}
          <ToDoList todos={this.state.todos} handleDeletList={(todoList)=>this.handleDeletList(todoList)}/>
          
          <div className='container'>
            {/* buton pentru afisarea formularului */}
            <button id='buttonForm' onClick={()=>this.handleIsFormDisplay()}>Afiseaza formularul</button>
          </div>
      </div>
    );
  }
